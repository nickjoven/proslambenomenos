...
2. **The Stribeck function.** The Kuramoto model uses $\sin(\psi - \theta)$ as the coupling function. The framework claims the vacuum has a Stribeck-type impedance. The [Renzo's Rule derivation §7.1](renzos_rule_from_kuramoto.md#71-deriving-the-interpolation-function-from-the-kuramoto-onset) shows that replacing $\sin(\cdot)$ with a Stribeck-weighted coupling function and setting $\delta = 1/2$ reproduces the MOND interpolation.
